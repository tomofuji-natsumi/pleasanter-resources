...
// 削除確認ダイアログの具体化（編集画面）
//
// #DeleteCommand クリック時、Pleasanter標準の確認ダイアログ（data-confirm属性による汎用メッセージ）の代わりに、
// 対象レコードのタイトルを含めた独自の確認ダイアログを表示する。
//
// ⚠️ 重要・要検証:
// - Pleasanter標準の確認処理がどのタイミング・方式（window.confirm/独自モーダル等）で動くのか、このリポジトリ内では未確認。
//   本スクリプトは「#DeleteCommandのクリックをcaptureフェーズで横取りし、標準の確認処理より先に独自ダイアログを出す」という前提で実装している。
// - 独自ダイアログで「OK」を選んだ場合のみ、__customConfirmedフラグを立てて再度クリックを発火させ、標準の削除処理（$p.send等）に処理を委ねる。
