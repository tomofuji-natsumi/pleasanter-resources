...
//
// FullCalendarのインスタンスに外部からアクセスする手段が無いため、
// ドラッグ移動時にPleasanterが送信するAjaxリクエスト
// （POST /items/{SiteId}/updatebycalendar、実機確認済み）を
// $.ajaxPrefilterで横取りする方式にしている。
//
// 汎用化のため、SiteId・レコードID・日付列名（Results_XXX）は
// すべてリクエスト自体から動的に取り出しており、特定サイト・
// 特定列名には依存しない。
//
// 処理の流れ:
// 1. updatebycalendar宛のリクエストを検知したら、元のリクエストは
//    いったん中止する（beforeSendでfalseを返す＝jQuery標準のキャンセル方法）
// 2. 対象サイトの全レコードを取得し、移動先の日付に別レコードが
//    既に存在するか確認する（ColumnFilterHashの日付絞り込みは他サイトの
//    実機検証でも不安定だったため、全件取得してクライアント側で比較する
//    holiday_setting.js等と同じ方式を採用）
// 3a. 衝突が無ければ、元のリクエストと同内容で送り直すだけ（通常の移動）
// 3b. 衝突があれば、確認ダイアログを出し、OKの場合のみ
//     「ドラッグしたレコード→移動先の日付」「衝突したレコード→
//     ドラッグしたレコードの元の日付」の2件を送信する（入れ替え）。
//     キャンセルした場合は何も送信しない
// 4. どの分岐でも、最終的に画面をリロードしてカレンダー表示を
//    正しい状態に同期する（FullCalendar側に再取得を指示する手段が
//    無いための代替策）
//
